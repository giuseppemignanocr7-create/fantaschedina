  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-2xl mx-auto px-3 py-3 space-y-3">
        ...
        <div className="relative overflow-hidden rounded-2xl p-4 border border-cyan-200 bg-gradient-to-br from-cyan-50 via-white to-cyan-50 animate-pop-in">
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute top-0 bottom-0 w-20 bg-white/50 animate-shine" />
          </div>
          <div className="absolute -right-4 -bottom-6 text-[80px] opacity-15 select-none animate-float">🎁</div>
          <div className="relative">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] text-cyan-700 uppercase tracking-widest">
              ...
              <div className="text-right flex-shrink-0">
                <p className="text-[10px] text-slate-500 uppercase tracking-widest">I tuoi gettoni</p>
                <p className="text-xl font-black text-yellow-700 mt-0.5">{profile?.coins ?? 0} 🪙</p>
              </div>
            </div>
            <div className="mt-3 space-y-2">
              {premiSettimana.map((premio, i) => (
                <div
                  key={premio.position}
                  className="flex items-center gap-3 rounded-xl bg-black/25 border border-slate-200 px-3 py-2 animate-slide-up"
                  style={{ animationDelay: `${i * 60}ms`, animationFillMode: 'backwards' }}
                >
                  <span className="text-lg">
                    {premio.position === 1 ? '🥇' : premio.position === 2 ? '🥈' : premio.position === 3 ? '🥉' : '🎖️'}
                  </span>
                  <span className="text-sm font-bold text-slate-900 flex-1">{premio.label}</span>
        ...
        {loading ? (
          ...
          <div className="glass-card p-8 text-center animate-pop-in">
            ...
            <p className="font-bold text-slate-900">L'albo d'oro ti aspetta!</p>
          ...
          <div className="space-y-2">
            {prizes.map((p, i) => {
              ...
              return (
                <div
                  key={i}
                  className="glass-card p-3 flex items-center gap-3 animate-slide-up hover:bg-slate-100 transition-colors"
                  style={{ animationDelay: `${i * 50}ms`, animationFillMode: 'backwards' }}
                >
                  <span className="text-2xl">{info.icon}</span>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-sm text-slate-900">Giornata {p.matchday}</p>
                    {p.podio && p.podio.length > 0 ? (
                      <div className="mt-1 space-y-0.5">
                        {p.podio.map(riga => (
                          <p key={riga.position} className="text-[11px] text-slate-500">
